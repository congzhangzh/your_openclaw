...
| 4 | **选择模型** | `openclaw config set agents.defaults.model "anthropic/claude-opus-4.6"` | 也可以选 MiniMax、GLM、Claude Code、Antigravity Opus 4.6——你决定。 |
...
继续往下读了解细节，或者直接跑完 7 步，好奇的时候再回来看。
...
```bash
# 更新 OpenClaw 到最新版
docker compose exec openclaw npm install -g openclaw@latest
docker compose restart openclaw
...
# 发送测试消息
docker exec -it openclaw openclaw message send --to +1234567890 --message "Hello from OpenClaw"

# 直接和助手对话
docker exec -it openclaw openclaw agent --message "Ship checklist" --thinking high

# 完整配置导出
docker exec -it openclaw openclaw config get
